...
export async function createProduct(p) {
  ...
  const { data, error } = await supabase.from("product").insert({
    ...
    continuous_use: !!p.continuous_use,
...
export async function generateShelves(location_id, letters, levels) {
  ...
  letters.forEach((L, li) => {
    if (L && !existing.has(L)) rows.push({ location_id, code: L, sort_order: (li + 1) * 100 });
    for (let n = 1; n <= levels; n++) { const code = (L || "") + n; if (!existing.has(code)) rows.push({ location_id, code, sort_order: (li + 1) * 100 + n }); }
  });
  if (!rows.length) return 0;
  const { error } = await supabase.from("storage_unit").insert(rows);
  if (error) throw error;
  return rows.length;
}
