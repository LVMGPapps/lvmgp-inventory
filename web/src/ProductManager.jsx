    ...
    <div className="overlay">
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="group">
          ...
          <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 14, marginTop: 10 }}>
            <input type="checkbox" style={{ marginTop: 3 }} checked={!!p.count_whole_only} onChange={(e) => set("count_whole_only", e.target.checked)} />
            <span>Don&apos;t count in <b>{p.usage_measure || "each"}</b> — count cases and {(p.package_unit || "package") + "s"} only.
              <div className="stat">The size unit still drives costing and $/{p.usage_measure || "each"}; it just won&apos;t appear as a box on the count screen.</div></span>
          </label>
        </div>
...
function Count({ products, locations, onhand, reload, onPending }) {
  ...
  // A–Z drops the shelf grouping, so each row carries its shelf (or location) as a chip instead.
  const shelfOf = (p) => loc
    ? (unitFor(p)?.unit_code || "")
...
function Users() {
  ...
  async function load() {
    setErr("");
    try { setRows(await db.listUsers()); }
    catch (e) { setRows([]); setErr(e.message || String(e)); }
  }
  useEffect(() => { load(); }, []);
  ...
  async function add() {
    ...
    setBusy(true);
    try { await db.createUser(email.trim(), password); setMsg(`Added ${email.trim()} — they can sign in with that password now.`); setEmail(""); setPassword(""); await load(); }
  ...
  return (
    <div>
      ...
      <div className="secthead" style={{ marginTop: 20 }}>People with access</div>
      {rows === null ? <div className="empty">Loading…</div> : rows.length === 0 ? <div className="empty">No users yet.</div> : (
        <div style={{ overflowX: "auto" }}>
          <table className="tbl" style={{ border: "none", minWidth: 420 }}>
            <thead><tr><th>Email</th><th>Last sign-in</th><th></th></tr></thead>
            <tbody>{rows.map((u) => (
              <tr key={u.id}>
                <td>{u.email}{!u.confirmed && <span className="stat"> · unconfirmed</span>}</td>
                <td className="stat">{u.last_sign_in_at ? new Date(u.last_sign_in_at).toLocaleDateString() : "never"}</td>
                <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                  <button className="mini" onClick={() => resetPw(u)}>Set password</button>{" "}
                  <button className="mini mini-danger" onClick={() => remove(u)}>Remove</button>
                </td>
              </tr>
            ))}</tbody>
          </table>
...
function Organize({ products, locations, reload }) {
  const [areaId, setAreaId] = useState("");
  const [bay, setBay] = useState(null);          // selected bay code when drilled in
  const [units, setUnits] = useState([]);
  ...
  useEffect(() => { loadUnits(); }, []);
  useEffect(() => { setBay(null); }, [areaId]);

  const aId = Number(areaId);
  const areaUnits = units.filter((u) => u.location_id === aId);
  const hasLetters = areaUnits.some((u) => /[A-Za-z]/.test(u.code));
  const letterOf = (code) => { const m = String(code).match(/^[A-Za-z]+/); return m ? m[0].toUpperCase() : null; };
  const bayLetters = [...new Set(areaUnits.map((u) => letterOf(u.code)).filter(Boolean))].sort();
  ...
  async function assign(pid, unitId) {
    setBusy(true);
    try { await db.setProductShelf(pid, aId, unitId); reload && reload(); }
    catch (e) { alert("Couldn't move: " + (e.message || e)); }
    finally { setBusy(false); setDrag(null); setOver(null); }
  }
  async function ensureBayUnit(L) {
    const u = areaUnits.find((x) => x.code.toUpperCase() === L && /^[A-Za-z]+$/.test(x.code));
    if (u) return u.storage_unit_id;
    const id = await db.addStorageUnit(aId, L, (L.charCodeAt(0) - 64) * 100);
    await loadUnits();
    return id;
  }
  ...
  async function generate() {
    ...
    try {
      const letters = [];
      const end = (lettersTo || "H").toUpperCase().charCodeAt(0);
      for (let c = 65; c <= end && c <= 90; c++) letters.push(String.fromCharCode(c));
      const n = await db.generateShelves(aId, letters.length ? letters : [""], Number(levels) || 5);
      await loadUnits(); alert(`Added ${n} shelves.`);
    } catch (e) { alert("Couldn't generate: " + (e.message || e)); }
    finally { setBusy(false); }
  }

  const dz = (key, onDropFn) => ({
    ...
    <div>
      <div className="toolbar">
        <select value={areaId} onChange={(e) => setAreaId(e.target.value)}>
          <option value="">Pick a storage area…</option>
          {locations.map((l) => <option key={l.location_id} value={l.location_id}>{l.name}</option>)}
        </select>
      ...
      {!areaId ? <div className="empty">Pick an area, then drag items onto a bay (A, B, C…). Click a bay to drop into its shelves (A1–A5).</div> : (
        <div>
          {areaUnits.length === 0 && (
            <div className="note" style={{ marginBottom: 12 }}>
              No shelves defined for {area(locations, aId)}. Generate a layout:
              <div className="toolbar" style={{ marginTop: 8 }}>
                <label className="stat">Bays A– <input style={{ width: 44 }} value={lettersTo} onChange={(e) => setLettersTo(e.target.value.slice(0, 1).toUpperCase())} /></label>
                <label className="stat">Shelves 1– <input style={{ width: 50 }} type="number" value={levels} onChange={(e) => setLevels(e.target.value)} /></label>
                <button className="btn btn-primary" disabled={busy} onClick={generate}>Generate</button>
                <button className="mini" disabled={busy} onClick={() => { setLettersTo(""); generate(); }}>Just shelves 1–{levels} (no letters)</button>
              </div>
            </div>
          )}
...
function area(locations, id) { return (locations.find((l) => l.location_id === id) || {}).name || "this area"; }
        ...
        <div>
          {(() => {
            ...
            return (
              <div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                  <button className="btn btn-primary" onClick={printGuide}>🖨 Print prep guide</button>
                  <button className="btn btn-ghost" onClick={() => window.open("/freezer-pull.html", "_blank")}>📋 Freezer pull sheet</button>
                  <button className="btn btn-ghost" onClick={() => window.open("/sausage-freeze-sop.html", "_blank")}>🌭 Sausage SOP</button>
                </div>
...
const stationOf = (c) => c.station || "pizza";
...
function builtLines(r, recipesById, compsById) {
  ...
  const sauceProducts = new Set(Object.values(compsById).filter((c) => c.kind === "sauce" && stationOf(c) === "pizza").map((c) => c.product_id));
...
function lineCostInfo(l, byId, seen) {
  ...
  if (isFreeLine(l)) return { cost: 0, src: "free" };            // tap water, ice — costs nothing but belongs on the recipe
  let subWhy = null;
  if (l.sub_recipe_id) {
...
const costChip = (c) => c.src === "free" ? <span className="bchip" style={{ background: "#F1F1F3", borderColor: "#B9BCC4", color: "#5b6069" }} title="deliberately free — tap water, ice">no cost</span>
  : c.src === "invoice" ? <span className="bchip" style={{ background: "#E6F4EF", borderColor: "#0E7C6B", color: "#0a5c50" }} title="what we last paid for it">invoice</span>
...
function MenuRecipes({ products, receipts }) {
  ...
  const table = (grp) => (
    <div style={{ overflowX: "auto" }}>
      <table className="tbl">
        <thead><tr>{reorder && <th style={{ width: 70 }}></th>}<th>Recipe</th><th>Price</th>{showCost && <><th>Cost</th><th>Food cost</th><th>Check</th></>}</tr></thead>
        <tbody>{grp.map((r, i) => {
  ...
  return (
    <div>
      ...
      {[...MENU_CATS, ...extra].map((c) => {
        ...
        if (c === "Wings") {
          const wb = grp.filter((r) => r.kind === "wing_base").sort((a, b) => Number(a.size_key) - Number(b.size_key) || ord(a, b));
          const ws = grp.filter((r) => r.kind === "wing_specialty"), wr = grp.filter((r) => r.kind !== "wing_base" && r.kind !== "wing_specialty");
          return (
            <div key={c} style={{ marginBottom: 18 }}>
              <div className="group-t">Wings — 1. Base wings</div>
              {wb.length ? table(wb) : <div className="stat">Mark a recipe as base wings (edit it → Type).</div>}
              {!q && <WingGuide comps={comps.filter((x) => stationOf(x) === "wings")} sizes={wb.length ? wb.map((b) => b.size_key).filter(Boolean) : WING_SIZES_DEFAULT} byId={byId} showCost={showCost} onEdit={(x) => setEditComp(x)} />}
              <div className="group-t" style={{ marginTop: 18 }}>Wings — 3. Flavored wings <span className="stat" style={{ textTransform: "none", letterSpacing: 0 }}>base wings + sauce + extras</span></div>
              {ws.length ? table(ws) : <div className="stat">No flavored wings yet.</div>}
              {wr.length > 0 && <><div className="group-t" style={{ marginTop: 18 }}>Wings — other</div>{table(wr)}</>}
            </div>);
        }
      ...
      {edit && <MenuRecipeEditor recipe={edit} products={products} byId={byId} recipes={rows} comps={comps} showCost={showCost} onClose={() => setEdit(null)} onSaved={() => { setEdit(null); load(); }} />}
      {editComp && stationOf(editComp) === "wings" && <WingComponentEditor comp={editComp} products={products} byId={byId} showCost={showCost} sizes={rows.filter((x) => x.kind === "wing_base").map((x) => x.size_key).filter(Boolean).sort((a, b) => a - b)} onClose={() => setEditComp(null)} onSaved={() => { setEditComp(null); load(); }} />}
      {editComp && stationOf(editComp) !== "wings" && <PizzaComponentEditor comp={editComp} products={products} byId={byId} showCost={showCost} onClose={() => setEditComp(null)} onSaved={() => { setEditComp(null); load(); }} />}
    </div>
  );
}
...
function PizzaGuide({ comps, byId, onEdit, showCost, prices, onSavePrices }) {
  ...
  const cell = (c, ratio, price) => {
    const l = componentLine(c, ratio), cost = lineCostInfo(l, byId), p = num(price);
    return <td><b>{l.portion || <span style={{ color: "#B0271B", fontWeight: 400 }}>no line measure</span>}</b>
      <div className="stat">{l.qty != null ? `${+l.qty.toFixed(2)} ${c.unit}` : "—"}</div>
      {showCost && <div className="stat" style={{ color: "#191B1F" }}>Cost {cost.cost != null ? money(cost.cost) : "—"}
        {p != null && cost.cost != null && <> · <span style={cost.cost / p > 0.3 ? { color: "#B0271B", fontWeight: 700 } : { color: "#0a5c50", fontWeight: 600 }}>{pct(cost.cost / p)} of {money(p)}</span></>}</div>}</td>;
  };
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="tbl" style={{ minWidth: 520 }}>
          ...
          <tbody>{sauces.map((c) => (
            <tr key={c.component_id} onClick={() => onEdit(JSON.parse(JSON.stringify(c)))} style={{ cursor: "pointer" }}>
              <td><b>{c.name}</b>{c.estimated && <div>{estChip}</div>}</td>
              {cell(c, 1, c.addon_price)}
              <td>{c.kids_qty != null ? <><b>{c.kids_tool || ""}</b><div className="stat">{c.kids_qty} {c.unit}{showCost ? ` · cost ${money(lineCostInfo(componentLine(c, 1, true), byId).cost)}` : ""}</div></> : <span className="stat">not set</span>}</td>
            </tr>))}</tbody>
        </table>
      </div>
      {addBtn("sauce")}
...
function ItemPicker({ productId, subId, free, products, excludeRecipeId, onPick, style }) {
  ...
  return (
    <select value={free ? "free" : subId ? `r:${subId}` : (productId ?? "")} onChange={(e) => onPick(e.target.value)} style={style}>
      <option value="">— not linked —</option>
      <option value="free">— no cost (tap water, ice) —</option>
...
// One-portion topping chart — used by the hot dog station (and anything else with a single portion).
function SimpleToppingGuide({ comps, station, title, intro, byId, onEdit, showCost }) {
  const list = comps.slice().sort((a, b) => a.sort - b.sort || a.name.localeCompare(b.name));
  return (
    <>
      <div className="group-t" style={{ marginTop: 18 }}>{title}</div>
      <div className="stat" style={{ marginBottom: 6 }}>{intro}</div>
      <div style={{ overflowX: "auto" }}>
        <table className="tbl" style={{ minWidth: 560 }}>
          <thead><tr><th>Topping</th><th>What the line uses</th><th>Amount</th>{showCost && <th>Cost</th>}<th>When</th></tr></thead>
          <tbody>{list.map((c) => { const l = componentLine(c, 1), cost = lineCostInfo(l, byId);
            return (
              <tr key={c.component_id} onClick={() => onEdit(JSON.parse(JSON.stringify(c)))} style={{ cursor: "pointer" }}>
                <td><b>{c.name}</b>{c.estimated && <div>{estChip}</div>}</td>
                <td><b>{l.portion || <span style={{ color: "#B0271B", fontWeight: 400 }}>no line measure</span>}</b></td>
                <td className="fig">{l.qty != null ? `${+l.qty.toFixed(2)} ${c.unit}` : "—"}</td>
                {showCost && <td className="fig">{cost.cost != null ? money(cost.cost) : "—"} {costChip(cost)}</td>}
                <td className="stat" style={{ marginTop: 0 }}>{(STAGES.find((x) => x[0] === (c.stage || "before")) || [])[1]}</td>
              </tr>); })}</tbody>
        </table>
      </div>
      <button className="mini" style={{ marginTop: 8 }} onClick={() => onEdit({ station, kind: "topping", name: "", unit: "oz", sort: 999, stage: "after", tool_kind: "text", estimated: true })}>+ Add topping</button>
    </>
  );
}

// Section 2 of the wing station: every sauce/seasoning at every wing count.
function WingGuide({ comps, sizes, byId, onEdit, showCost }) {
  const list = comps.slice().sort((a, b) => a.sort - b.sort || a.name.localeCompare(b.name));
  const sz = sizes.length ? sizes : WING_SIZES_DEFAULT;
  return (
    <>
      <div className="group-t" style={{ marginTop: 18 }}>Wings — 2. Sauces &amp; seasonings</div>
      <div className="stat" style={{ marginBottom: 6 }}>What each sauce adds on top of the base wings, by wing count: <b>what the line uses</b>, then the amount{showCost ? " and its cost" : ""}. Tap a sauce to edit its portions.</div>
      <div style={{ overflowX: "auto" }}>
        <table className="tbl" style={{ minWidth: 520 }}>
          <thead><tr><th>Sauce / seasoning</th>{sz.map((k) => <th key={k}>{k} wings</th>)}</tr></thead>
          <tbody>{list.map((c) => (
            <tr key={c.component_id} onClick={() => onEdit(JSON.parse(JSON.stringify(c)))} style={{ cursor: "pointer" }}>
              <td><b>{c.name}</b>{c.estimated && <div>{estChip}</div>}{!c.product_id && <div className="stat" style={{ color: "#B0271B" }}>not in inventory</div>}</td>
              {sz.map((k) => {
                const l = wingLine(c, k), cost = lineCostInfo(l, byId);
                if (l.qty == null) return <td key={k}><span className="stat">—</span></td>;
                return <td key={k}><b>{l.portion || <span style={{ color: "#B0271B", fontWeight: 400 }}>no line measure</span>}</b>
                  <div className="stat">{+l.qty.toFixed(2)} {c.unit}</div>
                  {showCost && <div className="stat" style={{ color: "#191B1F" }}>Cost {cost.cost != null ? money(cost.cost) : "—"}</div>}</td>;
              })}
            </tr>))}</tbody>
        </table>
      </div>
      <button className="mini" style={{ marginTop: 8 }} onClick={() => onEdit({ station: "wings", kind: "sauce", name: "", unit: "oz", sort: 999, portions: {} })}>+ Add sauce or seasoning</button>
    </>
  );
}

function WingComponentEditor({ comp, products, byId, sizes, onClose, onSaved, showCost }) {
  const [c, setC] = useState({ ...comp, portions: comp.portions || {} });
  const [busy, setBusy] = useState(false);
  const set = (k, v) => setC((s) => ({ ...s, [k]: v }));
  const setP = (size, k, v) => setC((s) => ({ ...s, portions: { ...s.portions, [size]: { ...(s.portions[size] || {}), [k]: v } } }));
  ...
  async function save() {
    ...
    setBusy(true);
    try { await db.savePizzaComponent({ ...c, station: "wings", kind: "sauce", portions }); onSaved(); } catch (e) { alert("Save failed: " + (e.message || e)); setBusy(false); }
  }
  async function remove() {
    if (!c.component_id) { onClose(); return; }
    if (!confirm(`Remove ${c.name}? Flavored wings using it will lose their sauce.`)) return;
    setBusy(true);
    ...
    <div className="overlay">
      <div className="sheet" onClick={(e) => e.stopPropagation()} style={{ width: "min(560px,100%)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2 style={{ marginBottom: 2 }}>{c.component_id ? c.name : "New wing sauce"}</h2>
          <button className="mini" onClick={onClose}>✕</button>
        </div>
        <div className="group">
          ...
          <div className="field" style={{ marginTop: 8 }}><label>Inventory item</label>
            <ItemPicker productId={c.product_id} subId={c.sub_recipe_id} free={num(c.fallback_cost) === 0 && !c.product_id && !c.sub_recipe_id} products={products} onPick={pick} />
        ...
        <div className="group">
          <div className="group-t">Portion by wing count</div>
          <div className="stat" style={{ marginBottom: 6 }}>Leave a size blank if this flavor isn't sold at that count.</div>
          {sz.map((k) => { const l = wingLine({ ...c, portions: { [k]: { qty: c.portions[k]?.qty, tool: c.portions[k]?.tool } } }, k), lc = lineCostInfo(l, byId).cost;
            return (
              <div key={k} style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end", marginBottom: 8 }}>
                <div style={{ width: 70, fontWeight: 700, paddingBottom: 8 }}>{k} wings</div>
                <div className="field" style={{ flex: "0 1 90px" }}><label>Amount ({c.unit})</label><input className="fig" type="number" step="any" value={c.portions[k]?.qty ?? ""} onChange={(e) => setP(k, "qty", e.target.value)} /></div>
                <div className="field" style={{ flex: "1 1 180px" }}><label>What the line uses</label><input value={c.portions[k]?.tool ?? ""} onChange={(e) => setP(k, "tool", e.target.value)} placeholder="e.g. 2 oz ladle" /></div>
                {showCost && <div className="stat" style={{ paddingBottom: 9 }}>{money(lc)}</div>}
              </div>); })}
...
function PizzaComponentEditor({ comp, products, byId, onClose, onSaved, showCost }) {
  ...
  async function save() {
    if (!String(c.name || "").trim()) { alert("Name it."); return; }
    setBusy(true);
    try { await db.savePizzaComponent({ ...c, station: stationOf(c) }); onSaved(); } catch (e) { alert("Save failed: " + (e.message || e)); setBusy(false); }
  }
  async function remove() {
    if (!c.component_id) { onClose(); return; }
  ...
  return (
    <div className="overlay">
      <div className="sheet" onClick={(e) => e.stopPropagation()} style={{ width: "min(560px,100%)" }}>
        ...
        <div className="group">
          <div className="group-t">{stationOf(c) !== "pizza" ? "Portion" : c.kind === "sauce" ? "Large portion" : "Single-topping portion (large)"}</div>
...
function MenuRecipeEditor({ recipe, products, byId, recipes, comps, onClose, onSaved, showCost }) {
  ...
  const sauces = (comps || []).filter((c) => c.kind === "sauce" && stationOf(c) === "pizza").sort((a, b) => a.sort - b.sort);
  const toppings = (comps || []).filter((c) => c.kind === "topping" && stationOf(c) === "pizza").sort((a, b) => a.sort - b.sort);
  const wingBases = (recipes || []).filter((x) => x.kind === "wing_base").sort((a, b) => Number(a.size_key) - Number(b.size_key));
  ...
  function changeUnit(l, unit) {
    ...
    setLine(l._k, { unit, factor: (p ? suggestFactor(p, unit) : null) ?? l.factor, fallback_cost: isFreeLine(l) ? 0 : null });
  }
  async function uploadPhoto(e) {
    const f = e.target.files?.[0]; if (!f) return;
    setBusy(true);
  ...
  function setKind(kind) {
    if (kind === "wing_specialty") { setR((s) => ({ ...s, kind, base_recipe_id: wingBases.some((b) => b.recipe_id === s.base_recipe_id) ? s.base_recipe_id : (wingBases[0]?.recipe_id || null), sauce_component_id: wingSauces.some((c) => c.component_id === s.sauce_component_id) ? s.sauce_component_id : (wingSauces[0]?.component_id || null) })); return; }
      ...
      <div className="sheet" onClick={(e) => e.stopPropagation()} style={{ width: "min(1040px,100%)" }}>
        ...
        <div className="group">
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            {r.category === "Wings" && <div className="field" style={{ flex: "1 1 150px" }}><label>Type</label>
              <select value={r.kind || "standard"} onChange={(e) => setKind(e.target.value)}>
        ...
        {isSpec && (
          <div className="group">
            ...
            {isWing ? (
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <div className="field" style={{ flex: "1 1 200px" }}><label>Base wings</label>
                  <select value={r.base_recipe_id ?? ""} onChange={(e) => set("base_recipe_id", num(e.target.value))}>
                    <option value="">— choose —</option>{wingBases.map((b) => <option key={b.recipe_id} value={b.recipe_id}>{b.name}</option>)}</select></div>
                <div className="field" style={{ flex: "1 1 200px" }}><label>Sauce / seasoning</label>
                  <select value={r.sauce_component_id ?? ""} onChange={(e) => set("sauce_component_id", num(e.target.value))}>
                    <option value="">None</option>{wingSauces.map((c) => <option key={c.component_id} value={c.component_id}>{c.name}</option>)}</select></div>
              </div>) : <>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            {isWing && (() => { const b = recipesById[r.base_recipe_id], sc = compsById[r.sauce_component_id];
            ...
            <table className="tbl" style={{ minWidth: 860 }}>
              ...
              <tbody>{r.lines.map((l, i) => {
                ...
                return (
                  <tr key={l._k}>
                    <td>
                      <ItemPicker productId={l.product_id} subId={l.sub_recipe_id} free={isFreeLine(l)} products={products} excludeRecipeId={r.recipe_id} onPick={(v) => pick(l, v)} style={{ width: "100%" }} />
                      <div className="stat">{sub ? subInfo(sub.recipe_id, showCost) : p ? <>#{p.product_id} · counted in {measure(p)}{showCost && <> · {cpu != null ? <>${cpu.toFixed(4)}/{measure(p)} <span style={{ color: paidInfo(p) ? "#0a5c50" : "#9a5b00" }}>({priceNote(p)})</span></> : <span style={{ color: "#B0271B" }}>no delivery or vendor price</span>}</>}</> : isFreeLine(l) ? <span>No cost — tap water, ice, or anything that isn't bought</span> : <span style={{ color: "#B0271B" }}>Pick the item this line uses</span>}</div>
